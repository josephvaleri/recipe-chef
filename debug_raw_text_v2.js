...
function showRawText(text, label) {
  ...
  for (let i = 0; i < Math.min(text.length, 200); i++) { // Limit to first 200 chars
    const char = text[i]
...
async function debugRawText() {
  try {
    console.log('Fetching global recipe ingredients...')
    
    // Get ingredients directly
    ...
    if (ingredients && ingredients.length > 0) {
      const rawName = ingredients[0].raw_name
