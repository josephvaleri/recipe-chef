// Test the full matching process for recipe 658 ingredients
// Run this in your browser console on the recipe chef site

async function testFullMatchingProcess() {
  try {
    console.log('🔍 Testing full matching process for recipe 658...')
    ...
    for (const rawIngredient of testIngredients) {
      ...
      for (const ingredient of allIngredients) {
        ...
        if (ingredientName === rawName) {
          ...
          bestScore = 1.0
          console.log(`   ✅ EXACT MATCH (original): "${ingredient.name}"`)
          break
        }
        ...
        if (ingredientName.includes(rawName) || rawName.includes(ingredientName)) {
          ...
          candidates.push({ ingredient, score, type: 'partial match' })
    ...
    console.log('\n' + '='.repeat(60))
    console.log('✅ Full matching process test complete!')
